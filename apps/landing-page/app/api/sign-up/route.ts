...
function rediractionToError(request: NextRequest, reason:string){
...
export async function GET(request:NextRequest){
  ...
  if(!token || !role) {
    return rediractionToError(request, "missing_signup_details");
  }

  if(!isSignupRole(role)){
    return rediractionToError(request, "invalid_role");
  }

  if(!secretKey){
    console.error("CLERK_SECRET_KEY is not configured.");
    return rediractionToError(request, "authentication_unavailable");
  }
  ...
  try {
    ...
    if(user.unsafeMetadata.role !== "role"){
      return rediractionToError(request, "role_mismatch");
    }
  ...
  try {
    ...
    if(!backendResponse.ok){
      ...
      return rediractionToError(request, "backend_signup_failed");
    }
